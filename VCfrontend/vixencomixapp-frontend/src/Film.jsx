import React from "react";

export default function Film() {
  return (
    <div className="film-container">
      <h3 className="film-header">The Vale of Wales is getting animated!</h3>
      
      <p className="indented-paragraph">
        On August 19th, 2019, The Vale of Wales was firstly posted on vowcomic.tumblr.com, 
        starting a journey still ongoing at 6 years and counting. Despite a small audience, 
        Vixen Comix wants to turn Amanda, Lili, Sal, and Vera into hand-drawn human-animated characters!
      </p>

      <p className="indented-paragraph">
        With a budget goal of $80,000 for cast, crew, and other necessities, Vixen Comix will 
        share updates and sneak peeks as we bring this story to life. Stay tuned for more 
        information on release dates and how you can watch the animation when it's ready! 
        We can't wait to share this new chapter of The Vale of Wales with all of you.
      </p>
    </div>
  );
}